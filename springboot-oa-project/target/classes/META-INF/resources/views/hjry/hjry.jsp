    ...
    <div class="row">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body form-group" style="margin-bottom:0px;">
                <label class="col-sm-2 control-label" style="text-align: right; margin-top:5px">汇交人员姓名：</label>
                <div class="col-sm-2">
                    <input type="text" class="form-control" name="hjryxm" id="search_hjryxm"/>
                </div>
...
<script type="text/javascript">
    ...
    $("#btn_add").click(function () {
        window.location.href = "${ctx}/hjry/addHjry?flag=1";
    })
    ...
    $("#btn_delete").click(function () {
        var ids = getIds();
        if (ids.length < 1) {
            layer.alert('请选择至少一个汇交人员!', {
                skin: 'layui-layer-lan'
                , closeBtn: 0
            })
        } else {
            layer.confirm('确定删除这些汇交人员吗?？', {
                btn: ['是的', '按错了'] //可以无限个按钮
            }, function () {
                //确定按钮的回调函数
                var url = "${ctx}/hjry/removeHjry";
                var params = {
                    flag: 2,
                    ids: ids
                }
                $.ajax({
                    ...
                    success:function(data){
                    	if(data=="success"){
                    		setTimeout(function(){
                    			$("#table").bootstrapTable("refresh", {url: '${ctx}/pjson'});
                    		},1000);
    ...
    function deleteone(id) {
       layer.confirm('确定删除这个汇交人员吗?', {
            btn: ['是的', '按错了'] //可以无限个按钮
        }, function () {
            //确定按钮的回调函数
            window.location.href = "${ctx}/hjry/removeHjry?flag=1&id=" + id;
        })
    };
        ...
        inint() {
            ...
            this.obj.bootstrapTable({
                ...
                columns: [
                    ...
                    {field: 'hjsf', title: '汇交省份'},
                    {field: 'hjryxm', title: '汇交人员姓名'},
                    {field: 'hjzw', title: '职位'},
                    {field: 'hjdz', title: '汇交人员地址'},
                    {field: 'hjyx', title: '邮箱'},
                    {field: 'hjlxdh', title: '联系电话'},
                    {
                        field: 'tool', title: '操作', align: 'center',
                        formatter: function (value, row, index) {
                            var element =
                                "<a  data-id='" + row.id + "' href='${ctx}/hjry/updateHjry?flag=1&id=" + row.id + "' class='btn btn-info btn-sm'>编辑</a> &nbsp;" +
